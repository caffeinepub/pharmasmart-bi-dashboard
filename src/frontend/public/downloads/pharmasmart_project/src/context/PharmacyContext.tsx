...
function seedInvoices(): Invoice[] {
  ...
  for (const customer of customers) {
    for (const purchase of customer.purchaseHistory) {
      const items: InvoiceItem[] = purchase.medicines.map((med) => ({
        medicineId: `seed-${med.name}`,
...
export function PharmacyProvider({ children }: { children: React.ReactNode }) {
  ...
  const totalProfit = useMemo(() => computeTotalProfit(invoices), [invoices]);
  ...
  function createInvoice(items: InvoiceItem[], customerName: string) {
    console.log(
      "[PharmaSmart] createInvoice called — items:",
      items,
      "customerName:",
      customerName,
    );
    ...
    setInvoices((prev) => {
      ...
      console.log(
        "[PharmaSmart] invoices updated — total:",
        updated.length,
        "new invoice profit:",
        invoice.items
          .reduce(
            (s, it) =>
              s + ((it.unitPrice || 0) - (it.costPrice || 0)) * (it.qty || 0),
            0,
          )
          .toFixed(2),
        "EGP",
    ...
    // Update customers after invoice creation — costPrice is stored on the item at time of sale
